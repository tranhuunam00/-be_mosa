    ...
    if (req.headers.authorization) {
      ...
      switch (user.role) {
        case enums.UserRole.CUSTOMER:
          ...
          if (!customer) {
            logger.debug(`[requireLogin]: find customer by user id -> ${httpResponses.CUSTOMER_NOT_FOUND}`);
            return res.status(httpResponses.HTTP_STATUS_UNAUTHORIZED).json({
              success: false,
        ...
        case enums.UserRole.SALLER:
          ...
          if (!saller) {
            logger.debug(`[requireLogin]: find customer by user id -> ${httpResponses.SALLER_NOT_FOUND}`);
            return res.status(httpResponses.HTTP_STATUS_UNAUTHORIZED).json({
              success: false,
