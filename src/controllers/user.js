...
const confirmRegister = async (req, res) => {
  try {
    ...
    const existedUser = await userService.updateUserByFilter({ _id: payload._id }, { isConfirm: true });
    if (!existedUser) {
...
const forgotPassword = async (req, res) => {
  try {
    ...
    if (!success) {
      logger.debug(`[forgotPassword] sendMail ->${httpResponses.SEND_MAIL_ERROR}`);
      return res.badRequest(httpResponses.SEND_MAIL_ERROR);
    }
...
const resetPassword = async (req, res) => {
  try {
    ...
    await userService.updateUserByFilter({ _id: user._id }, { password: hashPassword });
...
const updateUser = async (req, res) => {
  try {
    ...
    if (newModel.password && newModel.type == 'CHANGE_PASSWORD') {
      ...
      await userService.updateUserByFilter({ _id: user.id }, { password: hashPassword });
    ...
    if (req.files && req.files.avatar && req.files.avatar[0]) {
      const avatarUrl = await googleDriveService.uploadGgDrive(req.files.avatar[0]);
      newModel.avatarUrl = avatarUrl;
    }
    if (req.files && req.files.cover && req.files.cover[0]) {
      const coverUrl = await googleDriveService.uploadGgDrive(req.files.cover[0]);
